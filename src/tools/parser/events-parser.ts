...
export function parseEventsFromSql(content: string): EventsMap {
  ...
  const insertMatch = content.match(
    /INSERT INTO\s+`\#?\{\#\}?events`\s*\(([^)]+)\)\s*VALUES\s*([\s\S]*?);/i
  );
  ...
  const valueMatches = valuesBlock.matchAll(
    /\(\s*(\d+)\s*,\s*'([^']*)'\s*,\s*'([^']*)'\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/g
  );
...
if (require.main === module) {
  const sqlPath = path.resolve(
    process.env.INSTANTCMS_SOURCE || 'source',
    'install/languages/ru/sql/base.sql'
  );
