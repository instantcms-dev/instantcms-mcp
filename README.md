...
### Профиль инструментов (опционально)

Определения инструментов (`tools/list`) уходят в контекст модели на каждый запрос, поэтому выключение неиспользуемых групп прямо снижает расход токенов:

```bash
INSTANTCMS_MCP_DISABLE=templates,sources node dist/index.js
```

- Группы: `meta, generators, knowledge, database, sources, languages, extensions, project, templates`. Неизвестные группы игнорируются с предупреждением в лог. / Groups: `meta, generators, knowledge, database, sources, languages, extensions, project, templates`. Unknown groups are ignored with a log warning. / 分组：`meta, generators, knowledge, database, sources, languages, extensions, project, templates`。未知分组会被忽略并记录警告。
- Мета-инструменты (`find_tool`, `get_workflow`) могут упоминать выключенные инструменты — их вызов вернёт ошибку клиента. / Meta tools (`find_tool`, `get_workflow`) may still mention disabled tools — calling them returns a client error. / 元工具（`find_tool`、`get_workflow`）仍可能提及已关闭的工具——调用会返回客户端错误。
- Фактическое число активных инструментов показывает `get_server_capabilities`. / `get_server_capabilities` reports the actual number of active tools. / `get_server_capabilities` 显示当前实际启用的工具数量。
